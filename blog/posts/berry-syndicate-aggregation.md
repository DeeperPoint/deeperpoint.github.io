...
1. **Participant Fragmentation ($F_{endo}$):** Individually, Elena and her neighbors were sub-scale, facing a high scale wall. The platform fabricated a **virtual collective counterparty** on-demand, allowing them to act as a single Tier-One supplier to Market Kurly without the permanent overhead of a legal cooperative.
2. **Opacity ($F_{endo}$):** Traditional directories list only farm names. The platform's dynamic schemas mapped real-time variables—harvest windows, certified organic status, and volume capacity—making the latent supply visible to the global market.
